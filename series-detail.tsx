...
export const Route = createFileRoute("/series-detail")({
  validateSearch: search,
  head: () => ({ meta: [{ title: "STAR — סדרה" }] }),
...
function SeriesDetail() {
  ...
  if (!d) {
    return (
      <TvShell activePath="/series" initialRow={1} resetKey={`series-${id}`}>
      ...
      <div className="absolute inset-0">
        ...
        <div className="relative h-full flex flex-col justify-end px-16 pb-16">
          <div className="max-w-[50%] text-right">
            <div className="text-sm text-primary font-bold mb-2">סדרה{d.year ? ` · ${d.year}` : ""}</div>
            ...
            <div className="flex flex-row-reverse items-center gap-4 text-base text-white/80 mb-4">
              ...
              {d.episodes ? <span>{d.episodes} פרקים</span> : null}
            ...
            {d.cast.length > 0 && (
              <div className="mt-4 text-sm text-white/70">
                <span className="text-white/50">כיכוב: </span>{d.cast.slice(0, 4).join(", ")}
              </div>
            )}
            <div className="flex flex-row-reverse gap-3 mt-7">
              <Btn row={1} col={0} primary icon={<Play className="w-5 h-5 fill-current" />} label="נגן פרק 1" onSelect={() => nav({ to: "/player" })} />
